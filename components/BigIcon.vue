<template>
	<div :class="{'bg-surface-0 dark:bg-dark-900 relative flex size-10 items-center justify-center rounded-full border': border}">
		...
		<i v-if="statusColor" v-tooltip.top="capitalize(props.status.replaceAll('-', ' '))" class="pi pi-circle-fill border-surface-0 dark:border-dark-900 absolute right-[-3px] top-0 rounded-full border-2 text-2xs" :class="statusColor"/>
	</div>
</template>
