...
<script setup lang="ts">
	...
	const commands = {
		docker: {
			compact: `docker run -d --log-driver local --network host --restart=always --name globalping-probe -e GP_ADOPTION_TOKEN=${user.adoption_token} globalping/globalping-probe`,
			expanded: [
				...
				[ `-e GP_ADOPTION_TOKEN=${user.adoption_token} \\`, ' # Pass auto-adoption token' ],
				[ '--name globalping-probe globalping/globalping-probe' ],
			],
		},
		podman: {
			compact: `sudo podman run -d --cap-add=NET_RAW --network host --restart=always --name globalping-probe -e GP_ADOPTION_TOKEN=${user.adoption_token} globalping/globalping-probe`,
			expanded: [
