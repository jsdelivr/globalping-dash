<template>
	<div :class="{'border-surface-300 relative flex size-10 items-center justify-center rounded-full border': border}">
		...
		<i v-if="statusColor" v-tooltip.top="capitalize(props.status)" class="pi pi-circle-fill border-surface-0 absolute right-[-3px] top-0 rounded-full border-2 text-[0.6rem]" :class="statusColor"/>
	</div>
</template>
